...
* ✅ **テストで確認済みの範囲**
    * `tests/test_e2e_mock.py` で、Gemini呼び出し・クリップボード・通知をモック化したエンドツーエンド動作（デフォルトプロンプト選択→テンプレート展開→LLM結果コピー）を検証済み。
* ⚠️ **まだ実機で確認できていない/未完了の内容**
    * 実際のGemini APIへのリクエストや通知ウィンドウ表示は、外部依存のため自動テスト外（要APIキー・GUI環境での手動確認）。
    * ステップ5: モデル候補の`.env`読込やデフォルト指定は対応済みだが、その他細かなUX改善や検証は追加余地あり。
    * ステップ6: コンソール非表示実行（Windowsのバックグラウンド化）とログレベル切替のUI/設定は未着手。ロギングはファイル出力に限定され、同一プロセス内で再設定しない場合のみ想定。
    * ステップ7: PyInstaller等でのパッケージング手順と、セットアップ/トラブルシュートの最終ドキュメント化が未実施。
