...
4.  **ステップ4: デスクトップ通知機能の実装**
    *   **対象:** `ClipperAgent` (CLI)
    *   **内容:**
        *   Pythonのデスクトップ通知ライブラリを選定・導入 (例: `plyer`, `win10toast-click` など、Windowsで動作するもの)。
        *   メインアプリの処理成功時（クリップボードへのコピー完了後）に、「処理完了: [プロンプト名]の結果をクリップボードにコピーしました。」のようなメッセージをデスクトップ通知で表示。
        *   メインアプリの各種エラー発生時に、具体的なエラー内容をデスクトップ通知で表示。
        *   コンソールへのエラー出力は残しつつ、通知を追加する形とする。
    *   **動作確認:** 処理成功時および各種エラー発生時に、デスクトップ通知が表示されることを確認。

5.  **ステップ5: 設定アプリの機能拡充**
    *   **対象:** `ClipperAgentConfig` (GUI), `settings.json`, `.env`
    *   **内容:**
        *   プロンプト一覧で選択したプロンプトの内容を編集フォームに読み込む機能。
        *   「保存」ボタンで、既存プロンプトの場合は内容を更新する機能。プロンプト名の重複チェックを追加し、重複する場合はエラーメッセージを表示。
        ...
        *   GUIのレイアウト改善（仕様書のワイヤーフレームに近づける）。
    *   **動作確認:** プロンプトの編集、削除、デフォルト設定が正しく動作し、`settings.json` に反映されることを確認。利用可能モデルが `.env` から正しく読み込まれることを確認。
