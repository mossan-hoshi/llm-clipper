...
1.  **ステップ1: 設定ファイルの基礎構造と設定アプリのコア機能 (初期)** ✅ (Commit: ecccc47)
...
2.  **ステップ2: メインアプリの基本機能 (クリップボード入力とプロンプト組み立て)** ✅ (Commit: ecccc47)
    *   **対象:** `ClipperAgent` (CLI)
    *   **内容:**
...
3.  **ステップ3: メインアプリのGemini API連携とクリップボード出力** ✅ (Commit: ecccc47)
...
4.  **ステップ4: デスクトップ通知機能の実装** ✅ (Commit: ecccc47)
    *   **対象:** `ClipperAgent` (CLI)
    *   **内容:**
...
5.  **ステップ5: 設定アプリの機能拡充** ✅ (Commit: ecccc47)
...
6.  **ステップ6: メインアプリの機能拡充と仕上げ** ✅ (Commit: ecccc47)
    *   **対象:** `ClipperAgent` (CLI)
    *   **内容:**
        *   ✅ コマンドライン引数が指定されなかった場合に、`settings.json` の `default_prompt_name` に設定されたプロンプトを使用する機能。デフォルトプロンプトが未設定の場合はエラー通知。
        *   ✅ アプリケーション実行時にコマンドプロンプトウィンドウが表示されないようにする（パッケージング時に `pythonw` や `--noconsole` オプションで対応）。
        *   ✅ ロギング機能の実装:
    ...
    *   **動作確認:** 引数なしで実行した際にデフォルトプロンプトが使用されること、ログファイルが正しく生成・記録されることを確認済み。

7.  **ステップ7: リファクタリングと品質保証 (QA)** ✅ (Commit: 87c6b12)
    *   **対象:** 全体 (`clipper_agent`, `clipper_agent_config`, `tests`)
    *   **内容:**
        *   ✅ **設定ロジックの統合:** 現在 `clipper_agent/settings.py` と `clipper_agent_config/settings_manager.py` に分散・重複している設定読み込みロジックを統合し、共通モジュールとして利用する。これにより一貫性を保ち、保守性を向上させる。
        *   ✅ **単体テストの整備:**
            *   現状のテスト (`test_notification.py`) は環境依存（Tkinter）がありCI等で失敗するため、モックを活用したテストへの修正。
            *   コアロジック（プロンプト置換、設定読み書き、API呼び出しモック）のテストカバレッジ向上。
        *   ✅ **パス解決の堅牢化:** 実行環境（ソースコード実行 vs PyInstaller実行）によるパスの違いを吸収するロジックの確認と修正。
    *   **目的:** パッケージング前のコードベースの健全化と、将来的な機能追加（他LLM対応など）への土台作り。

8.  **ステップ8: パッケージングとドキュメント最終化** 🔳
    *   **対象:** 全体
    *   **内容:**
        *   🔳 PyInstallerなどのツールを使用して、`ClipperAgent.exe` と `ClipperAgentConfig.exe` をスタンドアロンの実行ファイルとしてパッケージングする手順を確立（`.spec`ファイルの作成）。
...
1.  **配布ファイルの取得:**
    *   `dist` フォルダ内の `ClipperAgent.exe` と `ClipperAgentConfig.exe` を任意のフォルダに配置してください。

2.  **APIキーの設定:**
    *   実行ファイルと同じディレクトリに `.env` ファイルを作成してください。
    *   `.env` ファイルにGemini APIキーと使用可能なモデルを記述します。
        ```
        GEMINI_API_KEY="your_api_key_here"
        AVAILABLE_MODELS="gemini-pro,gemini-1.5-flash"
        ```
...
1.  `ClipperAgentConfig.exe` を起動します。
2.  「一般設定」でログレベルを変更できます（通常はINFO）。
3.  「プロンプト編集」でプロンプト名、本文、利用モデルを入力し、「保存」ボタンを押してプロンプトを登録します。
    *   プロンプト本文には `${clipboard}` を含めることで、クリップボードの内容を挿入できます。
4.  登録済みプロンプト一覧から、頻繁に使用するものを選択し、「デフォルトに設定」ボタンを押します。

### 実行

1.  任意のテキストをコピーします。
2.  `ClipperAgent.exe` を実行します（ショートカットを作成し、ホットキーを割り当てることを推奨します）。
3.  デフォルトプロンプトが適用され、LLMによる処理結果がクリップボードに格納されます。
4.  特定のプロンプトを使用したい場合は、ショートカットのリンク先設定で引数を指定します。
    *   例: `ClipperAgent.exe "Translate to English"`

## 10. トラブルシューティング

*   **何も起きない:**
    *   ログファイル (`%APPDATA%\ClipperAgent\ClipperAgent.log`) を確認してください。
    *   APIキーが正しく設定されているか確認してください。
    *   デフォルトプロンプトが設定されているか、または引数でプロンプト名を指定しているか確認してください。
*   **エラー通知が表示される:**
    *   通知の内容に従って設定を確認してください。
